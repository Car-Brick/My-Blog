---
title: 五步前置编排器 + 三级路由
summary: 对话核心调度引擎：五步前置编排器（记忆装载→问题改写→文档路由→决策路由→执行计划组装）配合三级路由（聊天模式→文档范围→执行策略），层层递进确定最优执行模式
date: 2026-06-02
tags:
  - RAG
  - 架构
...
- 路由结果为空 或 候选为空 → `CLARIFICATION`（反问用户）
- 置信度 < 0.55 → `CLARIFICATION`（不够确定，需要确认）
- 候选 ≥ 2 且前两名分数差 ≤ 3 且属于不同知识范围 → `CLARIFICATION`（歧义，需要确认）
- 置信度 ≥ 0.55 → 选定 top-1 文档，继续 Step4
...
3. 判断路由：
   - 问"上一节/下一节/章节列表"且非分析类且单问题 → `GRAPH_ONLY`
   - 问"第X步/第X项"且非分析类 → `GRAPH_THEN_EVIDENCE`
   - 其他全部 → `RETRIEVAL`
...
1. **路由粒度递进**：先粗粒度判断"查不查文档"（Level1），再中粒度"查哪份文档"（Level2），最后细粒度"怎么查"（Level3），层层收窄
